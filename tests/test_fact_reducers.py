...
def test_each_fact_event_fills_exactly_its_slice() -> None:
    ...
    state = booted().then(pricing, issue, tow, trip, contact, done).state
...
def test_unrelated_events_leave_slices_alone() -> None:
    state = booted().then(PollTick()).state
...
def test_latest_fact_wins() -> None:
    state = (
        booted()
        .then(ContactRecorded(phone="555-000-1111"))
        .then(ContactRecorded(phone="555-222-3333"))
        .state
    )
